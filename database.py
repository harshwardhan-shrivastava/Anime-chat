...
def create_tables():
    ...
    # One-time generated persona per character (cached, shared by all users).
    cursor.execute("""
    ...
    """)
    # One active senpai conversation per user (overwritten on switch).
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS character_chat(
            ...
            last_switched_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
            PRIMARY KEY (user_id),
            FOREIGN KEY(user_id) REFERENCES users(id)
        )
    """)
...
def get_persona(character_id):
    """Return the cached persona row for a character, or None."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM character_personas WHERE character_id = ?",
        (str(character_id),),
    )
    row = cursor.fetchone()
    conn.close()
...
def save_persona(character_id, character_name, anime_name,
                 personality_traits, tone_descriptor, opening_line):
    """Insert (or replace) a persona row. One-time generation per character."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        """
        INSERT OR REPLACE INTO character_personas
            (character_id, character_name, anime_name,
             personality_traits, tone_descriptor, opening_line, generated_at)
        VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
        """,
        (str(character_id), character_name, anime_name,
         personality_traits, tone_descriptor, opening_line),
    )
    conn.commit()
    conn.close()
...
def get_character_chat(user_id):
    """Return the user's active senpai conversation row, or None."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM character_chat WHERE user_id = ?",
        (user_id,),
    )
    row = cursor.fetchone()
    conn.close()
...
def set_character_chat(user_id, character_id, character_name, anime_name,
                       conversation_history_json):
    """Create or overwrite the user's senpai conversation (switch)."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        """
        INSERT OR REPLACE INTO character_chat
            (user_id, character_id, character_name, anime_name,
             conversation_history, locked_at, last_switched_at)
        VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """,
        (user_id, str(character_id), character_name, anime_name,
         conversation_history_json),
    )
    conn.commit()
    conn.close()
...
def update_character_chat_history(user_id, conversation_history_json):
    """Append to the conversation history (called after each AI reply)."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE character_chat SET conversation_history = ? WHERE user_id = ?",
