...
// ========================================================
//  JOIN + MEMBERS — event delegation (always works)
// ========================================================
document.addEventListener("click", function (e) {
    // Join Community button
    var btn = e.target.closest("#joinCommunityBtn");
    if (btn) {
        e.preventDefault();
    ...
    // Members tab
    var tab = e.target.closest('[data-tab="members"]');
    if (tab) {
        var grid = document.getElementById("membersGrid");
        if (grid && !grid.dataset.loaded) {
            var slug2 = window.location.pathname.split("/").pop();
            fetch("/community/" + slug2 + "/members")
                .then(function (r) { return r.json(); })
                .then(function (d) {
                    if (!d.success || !grid) return;
                    var lbl = document.getElementById("membersCountLabel");
                    if (lbl) lbl.textContent = d.count + " members";
                    if (!d.members.length) { grid.innerHTML = '<p class="panel-placeholder">No members yet. Be the first to join!</p>'; return; }
                    var h = "";
                    d.members.forEach(function (m) {
                        var av = m.avatar && m.avatar !== "profile1.png" ? '<img src="/static/images/avatars/' + m.avatar + '" alt="">' : m.username.substring(0, 2).toUpperCase();
                        var jd = m.joined_at ? new Date(m.joined_at + "Z").toLocaleDateString() : "";
                        h += '<div class="member-card"><div class="member-avatar" style="background:' + (m.avatar_color || "#3b82f6") + '">' + av + '</div><div><div class="member-name">' + m.username + '</div>' + (jd ? '<div class="member-joined">Joined ' + jd + "</div>" : "") + "</div></div>";
                    });
                    grid.innerHTML = h;
...
(function () {
    ...
    function showAiMessage(text, chatBox) {
        if (!chatBox) return;
        var div = document.createElement("div");
        div.className = "ai-msg";
        var label = document.createElement("div");
        label.className = "ai-label";
        label.textContent = "\u{1F916} AnimeBot";
    ...
    function handleAiRequest(userText, chatBox, btn) {
        if (!userText.trim()) return;
        if (btn) btn.classList.add("loading");
        showAiMessage("Thinking...", chatBox);

        fetch("/api/ai-chat", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ message: userText, anime_slug: slug })
        })
            .then(function (r) { return r.json(); })
            .then(function (data) {
                if (btn) btn.classList.remove("loading");
                var aiBox = chatBox.querySelector(".ai-msg:last-child .ai-text");
                if (data.success && aiBox) {
                    aiBox.textContent = data.reply;
                } else if (aiBox) {
                    aiBox.textContent = data.error || "AI unavailable.";
                }
            })
            .catch(function () {
                if (btn) btn.classList.remove("loading");
                var aiBox = chatBox.querySelector(".ai-msg:last-child .ai-text");
                if (aiBox) aiBox.textContent = "Failed to reach AI.";
            });
    }

    // Community page AI button
    var aiBtn = document.getElementById("aiChatBtn");
    if (aiBtn) {
        aiBtn.addEventListener("click", function () {
            var input = document.getElementById("messageInput");
            var chatBox = document.querySelector(".chat-feed");
            var text = input ? input.value.trim() : "";
            if (!text) { input.focus(); return; }
            showAiMessage(text, chatBox);
            handleAiRequest(text, chatBox, aiBtn);
            if (input) input.value = "";
        });
    }

    // Modal AI button
    var modalAiBtn = document.getElementById("modalAiChatBtn");
    if (modalAiBtn) {
        modalAiBtn.addEventListener("click", function () {
            var input = document.getElementById("modalMessageInput");
            var chatBox = document.getElementById("modalChatBox");
            var text = input ? input.value.trim() : "";
            if (!text) { input.focus(); return; }
            showAiMessage(text, chatBox);
            handleAiRequest(text, chatBox, modalAiBtn);
            if (input) input.value = "";
        });
    }
})();
